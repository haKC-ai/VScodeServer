...
T2
  ...
  apt install -y nginx
# --------------------------------------------------------------------
# NGINX virtual-host – always points to the latest real LE certificate
# --------------------------------------------------------------------
TARGET_CERT_DIR="${REAL_CERT_DIR:-/etc/letsencrypt/live/${DOMAIN}}"
...
server {
    ...
    # --- real LetsEncrypt chain & key ---
    ssl_certificate     ${TARGET_CERT_DIR}/fullchain.pem;
    ssl_certificate_key ${TARGET_CERT_DIR}/privkey.pem;
    ...
    # redirect unauthenticated sessions to OAuth2-Proxy
    location @e {
...
# ░░░[ 🔐 Injecting OAuth2-Proxy Env with Dynamic code-server Port ]░░░

CODE_PORT=$(sudo journalctl -u code-server@hakcer -o cat | grep -oP 'HTTP server listening on http://127\.0\.0\.1:\K[0-9]+' | tail -1)

if [[ -z "$CODE_PORT" ]]; then
  echo "[!] Failed to detect code-server port — defaulting to 3000"
  CODE_PORT=3000
fi

sudo mkdir -p /etc/systemd/system/oauth2-proxy.service.d/

cat | sudo tee /etc/systemd/system/oauth2-proxy.service.d/env.conf >/dev/null <<EOF
[Service]
Environment="OAUTH2_PROXY_COOKIE_DOMAIN=vanta.hakc.ai"
Environment="OAUTH2_PROXY_COOKIE_SECURE=true"
Environment="OAUTH2_PROXY_REDIRECT_URL=https://vanta.hakc.ai/oauth2/callback"
Environment="OAUTH2_PROXY_PROVIDER=github"
Environment="OAUTH2_PROXY_CLIENT_ID=${OAUTH2_CLIENT_ID}"
Environment="OAUTH2_PROXY_CLIENT_SECRET=${OAUTH2_CLIENT_SECRET}"
Environment="OAUTH2_PROXY_EMAIL_DOMAINS=*"
Environment="OAUTH2_PROXY_UPSTREAMS=http://127.0.0.1:${CODE_PORT}"
EOF

echo "[*] Reloading systemd & restarting oauth2-proxy..."
sudo systemctl daemon-reexec
sudo systemctl daemon-reload
